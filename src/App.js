...
import {getObjectFromArrayByKeyValue} from "./util/helpers";
...
function App() {
  ...
  return (
    <div className="App" style={{backgroundColor: '#2B418B', height: '100vh', width: '100%'}}>
      <div style={{justifyContent: 'center', alignItems: 'center', position: 'relative', height: '100%', width: '100%'}}>
        <RouletteWheel
          allMovies={movies}
        />
      </div>
      <div style={{position: 'absolute', top: 20, right: 20, zIndex: 10}}>
        <Icon onClick={toggleSidebarVisible} name={'bars'} inverted/>
      </div>
      <MovieListSidebar
...
const MovieListSidebar = ({movies, setMovies, visible, toggleSidebarVisible}) => {
  const [animation, setAnimation] = useState();
  const [direction, setDirection] = useState();

  const [newMovie, setNewMovie] = useState({Title: ''});

  const [inputIcon, setInputIcon] = useState('add');

  const handleNewMovieChange = (event) => {
    event.preventDefault();
    setNewMovie({Title: event.target.value});
    setInputIcon('add')
  }
      ...
      <Grid textAlign={'center'} padded={'horizontally'}>
        <Grid.Column>
          <Grid.Row>
            ...
            <Form onSubmit={addMovie} style={{margin: 10}}>
              <Form.Input
            ...
            {movies.map((movie, index) => {
              return (
                <Grid.Row key={movie.Title}>
                  ...
                  <Grid.Column width={1}>
                    <Icon name='times' inverted link size={'large'} onClick={() => removeMovie(index)}/>
                  </Grid.Column>
                </Grid.Row>
              )
            })}
...
function RouletteWheel({allMovies}) {
  const wheelRef = useRef(null);
  const [mustSpin, setMustSpin] = useState(false);
  const [prizeNumber, setPrizeNumber] = useState(0);
  const [winningMovie, setWinningMovie] = useState({});
  const movieListLength = 8;

  const [currentDisplay, setCurrentDisplay] = useState(displays.SPIN);

  const generateMovieList = () => {
    console.log('generating list')
    let movieArr = [];
    let arr = [];
    while(arr.length < movieListLength){
      let r = Math.floor(Math.random() * (allMovies.length - 1)) + 1;
      if(arr.indexOf(r) === -1) {
        arr.push(r);
        let newMovie = {
          ...allMovies[r],
          option: r//allMovies[r].Title
        };
  ...
  const [movies, setMovies] = useState(generateMovieList());

  useEffect(() => {
    console.log('mounting')
    setMovies(generateMovieList());
  }, [])
  ...
  const handleSpinClick = () => {
    ...
    setCurrentDisplay(displays.ROULETTE);
  }

  /*useEffect(() => {
    setWinningMovie(movies[prizeNumber])
  }, [prizeNumber])*/

  const handleSpinComplete = (movie) => {
    setWinningMovie(getObjectFromArrayByKeyValue(allMovies, 'Title', movie));
    setCurrentDisplay(displays.ANNOUNCEMENT);
  }
  ...
  const resetDisplay = () => {
    setCurrentDisplay(displays.SPIN);
  }

  return (
    <div style={{width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center', position: 'relative', display: 'flex', flexDirection: 'column' }}>
      <RenderDisplay
        currentDisplay={displays.ROULETTE}
        wheelRef={wheelRef}
        winningMovie={winningMovie}
        setWinningMovie={handleSpinComplete}
      ...
      />
      <RenderDisplay
        currentDisplay={currentDisplay === displays.ANNOUNCEMENT ? displays.ANNOUNCEMENT : displays.SPIN}
        wheelRef={wheelRef}
        winningMovie={winningMovie}
        setWinningMovie={handleSpinComplete}
        handleSpinClick={handleSpinClick}
...
const RenderDisplay = ({currentDisplay, winningMovie, handleSpinClick, handleSpinEnd, mustSpin, prizeNumber, movies, resetDisplay, wheelRef, setWinningMovie}) => {
  ...
  useEffect(() => {
    if (wheelRef.current) {
      const newWheelSize = {
        width: wheelRef.current.offsetWidth,
        height: wheelRef.current.offsetHeight
      }
      console.log(newWheelSize);
  ...
  switch(currentDisplay) {
      ...
      return (
        ...
        <div style={{position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width: '100%'}}>
          <Button
            ...
            /*style={{
              width: 300, height: 150, fontSize: 36, /!*backgroundColor: '#F4DE11',*!/ fontFamily: 'Adventure', fontColor: '#672425',
    ...
    case displays.ROULETTE:
      return (
        /*<div style={{ position: 'absolute', top: 0, right: 0, width: '100%', height: '100%' }}>*/
        <div style={{position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%'}}>
          <div
            style={{
              /*height: 550, width: 700, */margin: 'auto', justifyContent: 'center', alignItems: 'center', backgroundColor: '#2B418B',
              display: 'flex'
            }}
          >
            <div style={{ height: wheelSize.height, width: wheelSize.width, margin: 'auto', justifyContent: 'center', alignItems: 'center', position: 'relative'/*, position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',*/ }}>
              {/*<div ref={wheelRef}>*/}
                {/*<Wheel
  ...
  return (
   /* <div style={{ position: 'absolute', top: 0, right: 0, width: '100%', height: '100%' }}>*/
      <div style={{position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', width: '100%'}}>
        ...
        >
          {(props.winningMovie && props.winningMovie.Title) ? props.winningMovie.Title : 'Unknown'}
        </Button>
      </div>
    /*</div>*/
  )
